...
def tilelang_prepare_mt_cp(
    ...
    num_tokens = T.dynamic("num_tokens")
    block_S = chunk_size
    num_sides = T.ceildiv(DK, block_N)
    ...
    if is_varlen:
        ...
        g_shape = (batch_size, num_tokens, H)
        b_shape = (batch_size, num_tokens, H)
    mt_shape = (batch_size, H, DK, DK)
    fallback_mask_shape = (batch_size, H)

    @T.prim_func
    def tilelang_prepare_mt_cp_kernel(
        k: T.Tensor(k_shape, dtype=qkva_dtype),
        a: T.Tensor(a_shape, dtype=qkva_dtype),
        g: T.Tensor(g_shape, dtype=g_dtype),
        b: T.Tensor(b_shape, dtype=b_dtype),
        ...
        num_warmup_chunks: T.Tensor([batch_size, H], dtype=seqlen_dtype),
        fallback_mask: T.Tensor(fallback_mask_shape, dtype=mask_dtype),
        mt: T.Tensor(mt_shape, dtype=mt_dtype),
    ):
        with T.Kernel(batch_size * H * num_sides, threads=256) as (bbhs,):
            bbh, side = bbhs // num_sides, bbhs % num_sides
            bb, bh = bbh // H, bbh % H
            bhg = bh // (H // Hg)
            ...
            seq_end_idx = cu_seqlens[bb + 1] if is_varlen else num_tokens
            num_iters = num_warmup_chunks[bb, bh]
            full_iters = T.ceildiv(seq_end_idx - seq_start_idx, block_S)
            calc_mt = fallback_mask[bb, bh] and num_iters >= full_iters
            seq_start_idx = seq_end_idx - num_iters * block_S
            ...
            x_shared = T.alloc_shared((block_S, DK), dtype=qkva_dtype)
            m_shared = T.alloc_shared((DK, block_N), dtype=qkva_dtype)
            z_shared = T.alloc_shared((block_S, block_N), dtype=qkva_dtype)

            x_fragment = T.alloc_fragment((block_S, DK), dtype=accum_dtype)
            m_fragment = T.alloc_fragment((DK, block_N), dtype=accum_dtype)
            z_fragment = T.alloc_fragment((block_S, block_N), dtype=accum_dtype)
            g_prod = T.alloc_fragment((1), dtype=accum_dtype)

            data_loaded = T.alloc_barrier(arrive_count=128)
            x_ready = T.alloc_barrier(arrive_count=128)
            mt_done = T.alloc_barrier(arrive_count=128)
            ...
            if tx < 128:
                T.set_max_nreg(X_NREG, 1)

                if calc_mt:
                    for i_s in T.serial(num_iters):
                        if i_s > 0:
                            T.barrier_wait(mt_done, (i_s - 1) % 2)
                        ...
                        T.barrier_arrive(x_ready)

            else:
                T.set_max_nreg(MT_NREG, 1)

                if calc_mt:
                    for j_k, j_n in T.Parallel(DK, block_N):
                        if j_k == side * block_N + j_n:
                            m_fragment[j_k, j_n] = 1
                        else:
                            m_fragment[j_k, j_n] = 0
                    g_prod[0] = 0

                    for i_s in T.serial(num_iters):
                        T.barrier_wait(data_loaded, i_s % 2)
                        T.barrier_wait(x_ready, i_s % 2)

                        g_prod[0] += g_shared[block_S - 1]
                        ...
                        T.barrier_arrive(mt_done)

                    g_prod[0] = T.exp2(g_prod[0] * 1.442695)
                    for j_k, j_n in T.Parallel(DK, block_N):
                        m_fragment[j_k, j_n] *= g_prod[0]
                    T.copy(
                        m_fragment,
                        mt[bb, bh, 0:DK, side * block_N : (side + 1) * block_N],
                    )

    return tilelang_prepare_mt_cp_kernel


def prepare_cp_correction(
    k: torch.Tensor,
    a: torch.Tensor,
    g: torch.Tensor,
    b: torch.Tensor,
    cu_seqlens: torch.LongTensor,
    num_warmup_chunks: torch.LongTensor,
    ...
    tilelang_prepare_mt_cp_kernel = tilelang_prepare_mt_cp(
        H,
        Hg,
        K,
        chunk_size,
        ...
        g_dtype=g.dtype,
        b_dtype=b.dtype,
        mt_dtype=final_correction.dtype,
        mask_dtype=fallback_mask.dtype,
        seqlen_dtype=cu_seqlens.dtype,
        accum_dtype="float32",
        is_varlen=is_varlen,
    )
    tilelang_prepare_mt_cp_kernel(
        k,
        a,
        g,
        b,
