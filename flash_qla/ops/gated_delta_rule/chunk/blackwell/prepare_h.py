...
def tilelang_prepare_h(
    ...
    def tilelang_prepare_h_kernel(
        ...
        with T.Kernel(batch_size * H, threads=512) as (bbh,):
            ...
            if use_tcgen05_s:
                mbar_s_L = T.alloc_barrier(arrive_count=[1] * 8)
                mbar_s_R = T.alloc_barrier(arrive_count=[1] * 8)
            ...
            if tx < 128:
                ...
                for i_s in T.serial(num_iters):
                    ...
                    if use_tcgen05_s:
                        ...
                        T.mbarrier_wait_parity(mbar_s_L[i_s % 8], (i_s // 8) % 2)
                        T.tcgen05_gemm(
                            x_shared,
                            y_shared[:, DV // 2 : DV],
                            h_tmem[:, DV // 2 : DV],
                        ...
                        T.mbarrier_wait_parity(mbar_s_R[i_s % 8], (i_s // 8) % 2)
                ...
                for i_s in T.serial(num_iters):
                    ...
                    if use_tcgen05_x:
                        T.tcgen05_gemm(
                            ...
                            k_shared[i_s % num_stages, :, 0 : DK // 2],
                            x_tmem_L,
                            transpose_A=True,
                            clear_accum=True,
                            mbar=mbar_x_L[i_s % 8],
                        ...
                        T.copy(x_tmem_L, x_fragment[:, 0 : DK // 2])
                        T.tcgen05_gemm(
                            a_shared[i_s % num_stages, :, :],
                            k_shared[i_s % num_stages, :, DK // 2 : DK],
                            x_tmem_R,
                            transpose_A=True,
                            clear_accum=True,
                            mbar=mbar_x_R[i_s % 8],
                        ...
                        T.copy(x_tmem_R, x_fragment[:, DK // 2 : DK])
                    else:
                        T.gemm(
                            a_shared[i_s % num_stages, :, :],
                ...
                for i_s in T.serial(num_iters):
                    ...
                    if use_tcgen05_y:
                        ...
                        T.copy(y_tmem_L, y_fragment[:, 0 : DV // 2])
                        T.tcgen05_gemm(
                            k_shared[i_s % num_stages, :, :],
                            h_shared[:, DV // 2 : DV],
                            y_tmem_R,
                        ...
                        T.copy(y_tmem_R, y_fragment[:, DV // 2 : DV])
                    else:
                        T.gemm(
                            k_shared[i_s % num_stages, :, :],
                            h_shared,
