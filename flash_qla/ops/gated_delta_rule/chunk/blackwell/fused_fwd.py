...
# Blackwell native experiment:
# Use explicit TCGEN05 GEMM calls so compilation fails instead of silently
# lowering to legacy HMMA. The next step is to move the long-lived accumulators
# to TMEM; for now this file establishes the strict native codegen boundary.
...
def tilelang_fused_chunk_gdr_fwd(
    ...
    def tilelang_fused_chunk_gdr_fwd_kernel(
        ...
        with T.Kernel(T.ceildiv(DV, block_DV) * batch_size * H, threads=512) as (bbhv,):
            ...
            mma_mbar_s = T.alloc_barrier(arrive_count=1)
            mma_mbar_v0 = T.alloc_barrier(arrive_count=1)
            mma_mbar_v1 = T.alloc_barrier(arrive_count=1)
            mma_mbar_o0 = T.alloc_barrier(arrive_count=1)
            mma_mbar_o1 = T.alloc_barrier(arrive_count=1)
            mma_mbar_o2 = T.alloc_barrier(arrive_count=1)
            ...
            if tx < 128:
                ...
                for i_s in T.serial(num_iters):
                    ...
                    T.mbarrier_wait_parity(mma_mbar_s, 0)
                ...
                for i_s in T.serial(num_iters):
                    ...
                    T.mbarrier_wait_parity(mma_mbar_v0, 0)
                    ...
                    T.mbarrier_wait_parity(mma_mbar_v1, 0)
                ...
                for i_s in T.serial(num_iters):
                    ...
                    T.mbarrier_wait_parity(mma_mbar_o0, 0)
                    ...
                    T.mbarrier_wait_parity(mma_mbar_o1, 0)
                    ...
                    T.mbarrier_wait_parity(mma_mbar_o2, 0)
                    T.copy(o_tmem[:, 0:block_DV], o_fragment)
                    T.barrier_arrive(bar_5)
